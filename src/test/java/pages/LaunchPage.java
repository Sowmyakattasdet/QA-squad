...
public class LaunchPage extends BasePage {
	...
	public void clickGetStartedButton() {
		...
		WebElement getStarted = wait
				.until(ExpectedConditions.elementToBeClickable(BtnGetStarted));
		getStarted.click();
	}

	public void clickStartYourPersonalizedJourneyButton() {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
		WebElement startper = wait.until(
				ExpectedConditions.elementToBeClickable(BtnStartPersonJny));
		startper.click();
	}
	...
	public boolean detailscycletrackingapp() {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		WebElement cycleTracking = wait
				.until(ExpectedConditions.visibilityOf(txtCycleTracking));
		cycleTracking.isDisplayed();
		return true;
	}
	public boolean imagesinsync() {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		WebElement imagedisplay = wait
				.until(ExpectedConditions.visibilityOf(imgWgtLoss));
		imagedisplay.isDisplayed();
		return true;
	}
	public boolean getstartednow() {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		WebElement getstdbtn = wait
				.until(ExpectedConditions.visibilityOf(BtnGetStarted));
		getstdbtn.isDisplayed();
		return true;
	}
	public boolean startyourpersonalized() {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		WebElement persjnybtn = wait
				.until(ExpectedConditions.visibilityOf(BtnStartPersonJny));
