...
public class EditYourProfileFunctionalTestingStepDefinition {
	...
	public void user_should_see_body_metric_section() {
		Assert.assertTrue(pom.getEditYourProfileFunctionalTestingPage().metricsTabDisplayed());
	}

	@When("User clicks on Preferences & Health tab")
	public void user_clicks_on_preferences_health_tab() throws InterruptedException {
		Thread.sleep(3000);
	...
	public void user_should_see_preferences_health_section() {
	  Assert.assertTrue(pom.getEditYourProfileFunctionalTestingPage().preferenceTabDisplayed());
	}
	...
	public void user_clicks_on_back_button_on_body_metric_section() {
	    pom.getEditYourProfileFunctionalTestingPage().clickBackButtonFrmMetricsTab();
	}

	@Then("User should see {string} section for basic tab")
	public void user_should_see_section_for_basic_tab(String string) throws InterruptedException {
	    Assert.assertTrue(pom.getEditYourProfileFunctionalTestingPage().basicinfoSectionDisplayed());
	}
	...
	public void user_should_see_success_message(String string) {
		String actualValueSaveMsg = pom.getEditYourProfileFunctionalTestingPage().getProfilSaveMsgTxt();
		String expectedValueSaveMsg= "Your profile information has been successfully updated.";
		Assert.assertEquals(actualValueSaveMsg, expectedValueSaveMsg, "Correct message not displayed");
	}
	...
	public void user_should_see_message(String string) throws InterruptedException {
		String actualPopupMsg = pom.getEditYourProfileFunctionalTestingPage().verifyPopupAlertDisplayed();
        String expectedPopmsg = "Enter medication name";
		Assert.assertEquals(actualPopupMsg, expectedPopmsg, "Correct label not displayed");
		LoggerFactory.getLogger().info("Enter medication name Label is visible.");

	   
	}

	@Then("Enter Medication name should be visible")
	public void enter_medication_name_should_be_visible() throws InterruptedException {
		String actualPopupMsg = pom.getEditYourProfileFunctionalTestingPage().verifyPopupAlertDisplayed();
        String expectedPopmsg = "Enter medication name";
		Assert.assertEquals(actualPopupMsg, expectedPopmsg, "Correct label not displayed");
		LoggerFactory.getLogger().info("Enter medication name Label is visible.");
	...
	public void should_display_empty_input_field_for_entering_medication_name() {
		...
		LoggerFactory.getLogger().info("Enter medication name input textbox is visible.");

	}

	@When("User clicks on Ok button after adding medication")
	public void user_clicks_on_ok_button_after_adding_medication() throws InterruptedException {
		
		pom.getEditYourProfileFunctionalTestingPage().dismissPopupAfterAddingMedicine();
	}
	...
	public void user_should_navigate_back_to_preferences_health_section_page() {
		Assert.assertTrue(pom.getEditYourProfileFunctionalTestingPage().preferenceTabDisplayed());	
		LoggerFactory.getLogger().info("User is on Preference tab");

	}

	@Then("User should see {string} info section for Body metric tab")
	public void user_should_see_info_section_for_body_metric_tab(String string) {
		pom.getEditYourProfileFunctionalTestingPage().metricsTabDisplayed();
		LoggerFactory.getLogger().info("Body Metrics label is displayed on Metrics tab");
