...
public class LoginStepDefinitions {
	...
	public void her_balance_logo_should_be_visible_on_the_top_left_corner() {
		Assert.assertTrue(loginPage.isLogoDisplayed(),
				"HerBalance logo is NOT visible");
	}
	...
	public void login_tab_should_be_active_by_default() {
		Assert.assertTrue(loginPage.isLoginTabActive(),
				"Login tab is NOT active by default");
	}
	...
	/* commented because of duplicate with SignUP */

	// @Then("Email ID input field with placeholder {string} should be visible")
	// public void
	// email_id_input_field_with_placeholder_should_be_visible(String
	// placeholder) {
	...
	public void sign_up_link_should_be_visible_and_navigates_to_the_sign_up_page() {
		Assert.assertTrue(loginPage.isTabVisible("Sign Up"),
				"Sign Up tab/link is NOT visible");

		loginPage.clickSignUpTab();

		Assert.assertTrue(loginPage.isSignUpTabActive(),
				"Sign Up tab did NOT become active. Current URL: "
	...
	public void a_testimonial_with_text_and_name_e_g_sarah_should_be_displayed(
			Integer age) {
	...
	public void there_should_be_exactly_tabs_login_and_sign_up(
			Integer expected) {
		Assert.assertEquals(loginPage.getTabsCount(), expected.intValue(),
				"Tabs count mismatch");
	}
	/* duplicate with signup */
	...
	// expected.intValue(), "Email+Password input count mismatch");
	// }
